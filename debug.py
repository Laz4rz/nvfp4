import torch
from nvfp4_gemv.submission import custom_kernel
...
def generate_input(
    ...
    for i, t in enumerate((a_ref, b_ref, sfa_ref, sfb_ref, sfa_permuted, sfb_permuted, c_ref)):
        print(f"{i}: {t.shape}, {t.dtype}")

    return (a_ref, b_ref, sfa_ref, sfb_ref, sfa_permuted, sfb_permuted, c_ref)


def run_single_case(m: int, k: int, l: int, seed: int) -> output_t:
    print(f"\nRunning case m={m}, k={k}, l={l}, seed={seed}")
    data = generate_input(m, k, l, seed)
    out = custom_kernel(data)
    print("Output shape:", tuple(out.shape))
    print("First few values:", out.flatten()[:8].cpu())
    return out
...
if __name__ == "__main__":
    ...
    for m, k, l, seed in cases:
        run_single_case(m, k, l, seed)
